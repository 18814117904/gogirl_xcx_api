...
@Service
public class SendXcxTemplateMsgService {
	...
	public Boolean sendScheduledMsg(String openid,Integer orderId,String time,String technicianName,String serveName,String storeName,String storePhone,String storeAddress) {
		...
		map.put("url", "pages/mine/serve-details?orderId="+orderId);
		map.put("time", time);
...
//	}	
	 ...
	 * 充值成功
	 */
	public Boolean sendChargeMsg(String openid,String time,String chargeAmount,String bestowAmount,String result,String balance,String storeName) {
		//获取且删除formId,
		XcxFormId xcxFormId =xcxFormIdMapper.selectFormIdByOpenid(openid,new Date(new Date().getTime()-new Long("604000000")));
		if(xcxFormId!=null){
			xcxFormIdMapper.deleteByPrimaryKey(xcxFormId.getId());
		}else{
			logger.info("没有formId,发不了充值成功提醒");
			return true;
		}
		...
		map.put("bestowAmount", bestowAmount);
		...
		logger.info(map.toString());
		try{
			myHttpPost.httpRequest(gogirlProperties.getGOGIRLMP()+"/gogirl_mp/wx/ma/sendChargeMsg", map);
		}catch(Exception e){
			logger.info(e.getMessage());
		}
		return true;
