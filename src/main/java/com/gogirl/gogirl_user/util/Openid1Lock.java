 ...
 */
public  class Openid1Lock {
	private Integer segments = 500;// 默认分段数量
	private final static HashMap<Integer, ReentrantLock> lockMap = new HashMap<>();

	/*静态内部类实现单例*/
    private static class SingletonHolder{
        private static final Openid1Lock instance = new Openid1Lock(null,true);
    }
    public static final Openid1Lock getInsatance(){
    ...
    private  Openid1Lock(Integer counts, boolean fair) {
		init(counts, fair);
	}
