 ...
 */
public  class IntegralOrderLock {
	private Integer segments = 2;// 默认分段数量
	private final static HashMap<Integer, ReentrantLock> lockMap = new HashMap<>();

	/*静态内部类实现单例*/
    private static class SingletonHolder{
        private static final IntegralOrderLock instance = new IntegralOrderLock(null,true);
    }
    public static final IntegralOrderLock getInsatance(){
        return SingletonHolder.instance;
    }
	/*静态内部类实现单例*/
	
    private  IntegralOrderLock() {
		init(null, false);
	}

    private  IntegralOrderLock(Integer counts, boolean fair) {
		init(counts, fair);
	}
